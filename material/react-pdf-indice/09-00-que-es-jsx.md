...
El equipo de Facebook también liberaron JSX al mismo tiempo que React, que provee de una
sintaxis para crear estructuras del DOM complejas con atributos. JSX es una extensión de JavaScript
que nos permite crear los componentes React usando una sintaxis similar a HTML (se podría decir
que es HTML incrustado en JS). La sintaxis de JSX nos permite visualizar de una forma mas rápida
la estructura de nuestro componente. Para usar React no es necesario usar JSX pero lo hace más
...
En JSX se pueden usar etiquetas HTML o componentes propios creados con React. Por convención
```
...
```tsx
atributos class y for se escriben como className y htmlFor en JSX. Y si los atributos pertenecen a
```
...
```
Esto se debe a que cuando transforma el código JSX a JS, React solo puede crear un elemento para
devolverlo.
